...
from .location_generator import (
    LocationGenerator,
)  # Adicionado para geração de localização
...
class GameEngine:
    ...
    def process_action(
        ...
        if details and details.strip():  # Adicionar apenas se houver detalhes
            game_state.add_message(role="user", content=details)

        game_state.current_action = action  # Registrar a ação original do jogador

        actual_handler = action_handler or get_action_handler(action)
        # O resultado do handler mecânico
        ...
        if (
            ...
            # Esta atualização é específica para o nome da localização.
            # A IA pode fornecer uma descrição mais detalhada posteriormente.
            game_state.current_location = result_from_handler["new_location"]

        # Se a ação original era para interpretação e o handler não a resolveu para uma ação específica,
        # a IA deve interpretar a entrada original do jogador.
        if action.lower() in ["custom", "interpret"] and not result_from_handler.get(
            "action_performed"
        ):
            action_for_ai = "interpret"
            message_for_ai_narration = details

        # Handle survival updates and append to AI narration message
        survival_message_part = self._handle_survival_updates(
            character, action_for_ai, game_state
        )
        if survival_message_part:
            message_for_ai_narration += survival_message_part
        ...
        should_call_ai = (
            ...
            and (
                ...
                or action_for_ai  # Usar action_for_ai, a ação efetiva para a IA
                in [
                    "look",
                    "search",
                    "talk",
        ...
        if should_call_ai:
            return self._handle_ai_interaction(
            ...
            # Se a IA não for chamada, a mensagem do handler (se houver) deve ser registrada
            # como uma mensagem do sistema para refletir o resultado mecânico.
            if result_from_handler.get("message"):
                game_state.add_message(
                    role="system",
                    content=f"[Mecânica] {result_from_handler['message']}",
                )
            return result_from_handler
