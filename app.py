...
class GameApp:
    ...
    def _register_routes(self):
        ...
        # Utility routes
        self.app.route('/change_language', methods=['GET'])(self.change_language)
    ...
    def character(self):
        ...
        try:
            if request.method == 'POST':
                ...
                GameLogger.log_game_action('character_created', f"Name: {character.name}, Class: {character.character_class}", user_id)
            ...
            flash(get_text('errors.invalid_input', session.get('language'), str(e)), 'error')
            ...
            flash(get_text('errors.unexpected', session.get('language'), str(e)), 'error')
            return render_template('character.html', character=None, error=str(e))
    ...
    def process_action(self):
        ...
        try:
            ...
            # Process action
            result = self.game_engine.process_action(action, details, character, game_state, self.groq_client)
            ...
            GameLogger.log_game_action(action, f"ValueError: {str(e)}", session['user_id'], 'error')
            return self._error_response('invalid_input', str(e))
        except Exception as e:
            GameLogger.log_game_action(action if 'action' in locals() else 'unknown_action', 
                                f"Unexpected error: {str(e)}", session['user_id'], 'error')
            logger.error(traceback.format_exc())
    ...
    def reset_game(self):
        ...
        try:
            ...
            try:
                ...
                GameLogger.log_game_action('reset_game', f"Error deleting character: {str(e)}", session['user_id'], 'error')
            ...
            try:
                ...
                GameLogger.log_game_action('reset_game', f"Error deleting game state: {str(e)}", session['user_id'], 'error')
            ...
            GameLogger.log_game_action('reset_game', f"Unexpected error: {str(e)}", session['user_id'], 'error')
    ...
    def _create_character_from_form(self, character_data: Dict[str, Any]) -> Character:
        ...
        """
        # Process attributes and create character
        attributes = self._get_character_attributes(character_data)
        ...
        return Character(
            ...
            max_hp=attributes.get('max_hp', 20),
            current_hp=attributes.get('current_hp', 20),
            max_stamina=attributes.get('max_stamina', 10),
            current_stamina=attributes.get('current_stamina', 10),
            gold=attributes.get('gold', 50),
            experience=attributes.get('experience', 0),
            level=attributes.get('level', 1),
            inventory=attributes.get('inventory', ["Basic Sword", "Health Potion"])
        )
    
    def _get_character_attributes(self, character_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        """
        # This method is now handled by _create_character_from_form
        return {}
    ...
    def _load_game_state_with_language(self, user_id: str) -> GameState:
        ...
        return GameStateManager.load_game_state_with_language(self.game_engine, user_id, language)
    ...
    def _log_game_action(self, action: str, details: str = '', user_id: str = None, level: str = 'info') -> None:
        """
